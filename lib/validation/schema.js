...
function onlyRowsInTable(opts: Options, change: Change, context: Object) {
    ...
    if (invalids.size === context.node.nodes.size) {
        ...
        //     if (selection.hasEdgeIn(invalid)) {
        //         focusRow = true;
        //     }
        //     return change.removeNodeByKey(invalid.key, { normalize: false });
        // });
        // // Create new row
        // const newRow = createRow(opts, 1);
        // change.insertNodeByKey(context.node.key, 0, newRow, {
        //     normalize: false
        // });
        // // Avoid invalid selection (pointing on a deleted node)
        // if (focusRow) {
        //     change.moveToStartOf(newRow);
        // }
    }
