...
function onBackspace(
    ...
    // This behavior is to compensate this bug
    //
    const ignoreFirstCell =
        value.selection.collapseToStart().isAtEndOf(cells.first()) &&
        !blocks.first().isVoid;
    const ignoreLastCell =
        value.selection.collapseToEnd().isAtStartOf(cells.last()) &&
        !blocks.last().isVoid;
